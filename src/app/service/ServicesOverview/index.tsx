...
const services = [
  {
    ...
    items: ["Responsive Design","Progressive Web App","E-Commerce Solution","Content Management Systems","API Integration","Performance Optimization"],
  },
  {
    icon: <Palette className="w-6 h-6 text-[#4758E0]" />,
    title: "UI/UX Design",
    description: "Beautiful, intuitive interfaces that enhance user experience.",
    items: ["User Research & Testing","Wireframing & Prototyping","Visual Design","Design Systems","Mobile App Design","Brand Identity"],
  },
  {
    ...
    items: ["Market Analysis","Conversion Optimization","Analytics & Reporting","A/B Testing","Customer Journey Mapping","Growth Hacking"],
  },
  {
    ...
    items: ["SEO Optimization","Content Marketing","Social Media Management","Email Campaigns","PPC Advertising","Influencer Marketing"],
  },
  {
    icon: <Database className="w-6 h-6 text-[#4758E0]" />,
    title: "Backend Development",
    description: "Robust, scalable backend systems that power your applications.",
    items: ["Database Design","API Development","Cloud Infrastructure","Microservices","Server Optimization","Data Migration"],
  },
  {
    ...
    items: ["Security Audits","Penetration Testing","GDPR Compliance","Data Encryption","Access Control","Disaster Recovery"],
  },
];
        ...
        {services.map((service, index) => {
          ...
          return (
            <div
              key={index}
              data-index={index}
              className={`service-card border border-gray-200 rounded-2xl p-6 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-300 bg-white ${isVisible ? `fade-up ${delayClass}` : "opacity-0"}`}
            >
              <div className="mb-4 flex items-center gap-2">
                {service.icon}
                <h3 className="text-xl font-semibold text-[#131927]">{service.title}</h3>
              </div>
              <p className="text-[#555] text-sm sm:text-base leading-relaxed mb-4">{service.description}</p>
